...
/**
 * Extra pods declared alongside a kit. mParticle-Rokt 9.3+ already requires
 * Rokt-Widget ~> 5.3 transitively; declaring it directly keeps the Rokt iOS floor
 * visible in the Podfile and survives a lockfile that still holds a 5.2.x build.
 */
const KIT_COMPANION_PODS: Record<string, PodDeclaration[]> = {
...
function getPodDeclaration({ name, version }: PodDeclaration): string {
  return version ? `  pod '${name}', ${version}` : `  pod '${name}'`;
}

/**
 * Every pod a kit selection needs in the Podfile: the kit itself plus its companions.
 */
function getKitPods(kits: string[]): PodDeclaration[] {
  const pods: PodDeclaration[] = [];
  for (const kit of kits) {
    pods.push({ name: kit, version: KIT_VERSION_REQUIREMENTS[kit] });
    pods.push(...(KIT_COMPANION_PODS[kit] ?? []));
  }
  return pods;
}

/**
 * Declare each kit and its companion pods inside the main target block.
 *
 * Pods are matched individually rather than per kit selection: a Podfile generated by an
 * earlier plugin version already declares the kit, and must still pick up companion pods
 * added since (for example the `Rokt-Widget` floor).
 */
export function addKitPodsToPodfile(
  podfileContent: string,
  iosKits: string[]
): string {
  const missingPods = getKitPods(iosKits).filter(
    pod => !podfileContent.includes(`pod '${pod.name}'`)
  );
  ...
  const kitPods = missingPods.map(getPodDeclaration).join('\n');
...
`;
      ...
      if (props.iosKits && props.iosKits.length > 0) {
        podfileContent = addKitPodsToPodfile(podfileContent, props.iosKits);
      }
